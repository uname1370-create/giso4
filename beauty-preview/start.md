...
- [x] **Step 2 (Safety Check - Modal/Form):**
...
- [x] Pass facial characteristics (face shape, skin tone) to Step 3 so the UI can say: "فرم صورت شما تحلیل شد...".
...
- [x] Update `POST /api/generate` to accept the 3 subjective answers from the frontend.
- [x] Update `buildEnglishPrompt` or `styleDesignSpec` in `src/style-dna.ts`:
...
- [x] Test the integration with Cloudflare FLUX to ensure prompts are dynamically updated.
...
- [x] Create `POST /api/appointments` endpoint.
- [x] Accept payload: Customer info, selected style, base64/URL of the original photo, and base64/URL of the AI preview.
- [x] For now, securely save this to `data/leads.json` (as a bridge to Phase 5).
...
- [x] Setup unified lead schema and appointment models.
- [x] Create schemas: `Customer`, `SimulationSession`, `Appointment`.
- [x] Ensure photos are stored securely (e.g., S3 or local secure folder with 24-hour expiration policy for privacy).
...
- [x] Add Service Selector at Step 0.5 (Before Upload): Choose Brows, Lips, Eyes, or Removal.
- [x] Python Vision Engine Updates:
  - Add lip contour masking (`vision-engine/face/lips.py`).
  - Add eyeliner/eye masking.
- [x] Next.js Updates: Add specific styles and questions for Lips and Eyes.
- [x] *Special Rule for Removal:* Removal flow does NOT generate an AI image. It only analyzes the old tattoo and routes the user directly to the booking/human consultation step.
...
- [x] Add a floating Chat Widget UI.
- [x] Connect to OpenAI SDK (`src/providers/http.ts`).
- [x] Inject System Prompt: "You are Asal Rajabi's assistant. Tone: warm, feminine, professional. Do not guarantee medical results. Do not give final pricing."
...
- [x] Build CRM View: List of leads, phone numbers, and download buttons for Before/After photos.
- [x] Reference Manager: UI to upload/replace reference photos for each technique.
- [x] Global toggle to Enable/Disable the AI Assistant.
