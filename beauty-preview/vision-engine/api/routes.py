...
def process(req: ProcessRequest) -> ProcessResponse:
    ...
    if service != "eyebrows":
        raise HTTPException(
            status_code=400,
            detail="Only the active eyebrows service is enabled in this version.",
        )
    ...
    try:
        ...
        # 1) Detect the target on the ORIGINAL image.
        mask = build_target_mask(original, service)

        # 2) Undo any provider-wide geometry drift before measuring or compositing.
        aligned_edited = align_edited_to_original(original, edited)

        # 3) Measure raw provider changes after geometry normalization.
        provider_score = validate_provider_isolation(original, aligned_edited, mask)

        # 4) Hard composite: only the eyebrow mask may come from the AI result.
        result_bytes, coverage = composite_target(original, aligned_edited, mask)
        ...
        raise HTTPException(
            status_code=422,
            detail=f"vision processing failed: {type(exc).__name__}",
        ) from exc
    ...
    if provider_score < 0.70:
        raise HTTPException(
            status_code=422,
            detail="AI provider changed too much outside the detected eyebrow region; result rejected for safety.",
        )
    if provider_score < 0.88:
        warnings.append(
            "AI provider changed some non-target pixels; final compositing preserved the original outside the mask."
        )
